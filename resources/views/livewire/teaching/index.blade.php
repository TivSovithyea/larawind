    ...
    <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5 flex">
            ...
            <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 font-medium text-gray-900">Teacher</th>
                    ...
                    @foreach ($data as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                                <div class="text-sm">
                                    <div class="text-gray-400">{{ $item->day_label }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4">{{ $item->studyTime->name }}</td>
                            <td class="px-6 py-4">{{ $item->studyClass->classRoom->name }}</td>
        ...
        <div style="width: 100%" class="justify-center">
            <div class="flex flex-col justify-center">
                ...
                <div class='w-full md:w-1/2 px-3 mb-6'>
                    <label>Class</label>
                    <select wire:model='study_class_id'
                        class='appearance-none block w-full bg-white text-gray-700 border border-gray-400 shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-gray-500'
                        id='grid-text-1' type='text' placeholder='class' name="study_class_id" required>
                        <option value="">Choose</option>
                        @foreach ($study_classes as $study_class)
                            <option value="{{$study_class->id}}">{{$study_class->classRoom->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class='w-full md:w-1/2 px-3 mb-6'>
                    <label>Time</label>
                    <select wire:model='study_time_id'
                        class='appearance-none block w-full bg-white text-gray-700 border border-gray-400 shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-gray-500'
                ...
                <div class='w-full md:w-1/2 px-3 mb-6'>
                    ...
                    <select wire:model='day'
                        class='appearance-none block w-full bg-white text-gray-700 border border-gray-400 shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-gray-500'
                        id='grid-text-1' type='text' placeholder='day' name="day" required>
                        <option value="">Choose</option>
