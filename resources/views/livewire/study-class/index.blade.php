                    ...
                    @foreach ($data as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                                <div class="text-sm">
                                    <div class="text-gray-400">{{ $item->classRoom->name }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4">{{ $item->studyYear->name }}</td>
            ...
            <div class="flex flex-col justify-center">
                ...
                <div class='w-full md:w-1/2 px-3 mb-6'>
                    ...
                    <select wire:model='class_room_id'
                        class='appearance-none block w-full bg-white text-gray-700 border border-gray-400 shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-gray-500'
                        id='grid-text-1' type='text' placeholder='class' name="class_id" required>
                ...
                <div class='w-full md:w-1/2 px-3 mb-6'>
                    ...
                    <select wire:model='study_year_id'
                        class='appearance-none block w-full bg-white text-gray-700 border border-gray-400 shadow-inner rounded-md py-3 px-4 leading-tight focus:outline-none  focus:border-gray-500'
                        id='grid-text-1' type='text' placeholder='study year' name="study_year_id" required>
                        <option>Choose</option>
                        @foreach ($study_years as $study_year)
                            <option value="{{$study_year->id}}">{{$study_year->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class='w-full md:w-1/2 px-3 mb-6'>
